...
def add_rows(table,id, textOriginal,textDisplay,authorDisplayName,viewerRating,likeCount,publishedAt):
    table.add_row(id, textOriginal,textDisplay,authorDisplayName,viewerRating,likeCount,publishedAt)

def main():
    table = make_table()
    add_rows(table, "1", "Original text", "Display text", "Author", "Rating", "Likes", "Published")
    console = Console()
    console.print(table)
